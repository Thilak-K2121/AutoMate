...
### 19. Terminated State (Cold Start) Notification Deep-Linking & Intent Filter Fix
* **Files:** [`frontend/flutter_app/android/app/src/main/AndroidManifest.xml`](file:///c:/Users/user/Desktop/AutoMate/AutoMate/frontend/flutter_app/android/app/src/main/AndroidManifest.xml), [`frontend/flutter_app/lib/services/fcm_service.dart`](file:///c:/Users/user/Desktop/AutoMate/AutoMate/frontend/flutter_app/lib/services/fcm_service.dart), [`frontend/flutter_app/lib/screens/home_page.dart`](file:///c:/Users/user/Desktop/AutoMate/AutoMate/frontend/flutter_app/lib/screens/home_page.dart)
* **Rationale:**
  1. Added `<action android:name="FLUTTER_NOTIFICATION_CLICK" />` inside `MainActivity`'s intent filters so Android launches `MainActivity` when a notification banner is tapped while the app is closed or in background.
  2. Implemented `pendingNotificationPayload` queue in `FcmService` and connected `checkAndHandlePendingNotification()` inside `HomePage.initState()`. This resolves the race condition with `SplashScreenPage`'s authentication check, ensuring cold-started apps land smoothly on `HomePage` and instantly push the target `ChatPage` or `MetroRideDetailsPage` on top of the navigation stack.
